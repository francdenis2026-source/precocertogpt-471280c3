...
export function HomePremium() {
  ...
  return (
    <div className="pcx-home">
      ...
      <footer className="pcx-footer"><div className="pcx-shell pcx-footer__inner"><Link className="pcx-brand" to="/"><span className="pcx-brand__mark"><TrendingDown aria-hidden="true" /></span><span>preço<strong>certo</strong></span></Link><nav aria-label="Links do rodapé"><Link to="/sobre">Sobre</Link><Link to="/contato">Contato</Link><Link to="/privacidade">Privacidade</Link><Link to="/favoritos">Favoritos</Link><Link to="/lojista">Lojistas</Link></nav><div className="pcx-footer__meta"><small>© {new Date().getFullYear()} PreçoCerto</small><button className="pcx-footer__credit pc-dev-credit" type="button" aria-haspopup="dialog" aria-label="Sobre o desenvolvedor Franc D'nis" onClick={() => window.dispatchEvent(new Event("precocerto:developer-about"))}>dev&gt;<strong>Franc D&apos;nis</strong></button></div></div></footer>
