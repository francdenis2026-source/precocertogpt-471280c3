  ...
  return <div className="pdp-page">
    ...
    <main id="conteudo-principal" className="pdp-shell">
      ...
      <section className="pdp-lower-grid">
        ...
        <article className="pdp-panel pdp-similar"><header><h2>Produtos similares</h2></header>{similar.length ? <div>{similar.map(item => <Link to={`/produto/${item.slug || item.id}`} key={item.id}><ProductImage product={item} /><span><strong>{item.name}</strong><small>{item.size || item.unit}</small><b>{brl.format(item.minPrice)}</b><em>{item.establishment}</em></span><button type="button" aria-label={`Adicionar ${item.name} à lista`} onClick={event => { event.preventDefault(); event.stopPropagation(); void addToBasket(item); }}><ShoppingBasket /></button></Link>)}</div> : <p>Nenhum produto similar disponível nesta categoria.</p>}</article>
        <aside className="pdp-panel pdp-basket"><header><h2>Sua lista de compras</h2><span>{basket.reduce((sum,item)=>sum+item.quantity,0)} itens</span></header>{basketRows.slice(0,3).map(({entry,product:item}) => item && <div className="pdp-basket-row" key={entry.productId}><span>{item.name}</span><strong>{brl.format(item.minPrice * entry.quantity)}</strong></div>)}{!basketRows.length && <p>Sua lista ainda está vazia.</p>}<footer><span>Total estimado</span><strong>{brl.format(basketTotal)}</strong></footer><Link to="/cesta-basica">Ver lista completa</Link></aside>
      </section>
