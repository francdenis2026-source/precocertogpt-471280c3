...
export function HomePremium() {
  ...
  useLayoutEffect(() => {
    const form = searchAreaRef.current?.querySelector(".pcx-search");
    if (!form) return;
    const pinPoint = form.getBoundingClientRect().top + window.scrollY;
    const updatePinnedSearch = () => {
      const headerHeight = document.querySelector(".pcx-header")?.getBoundingClientRect().height ?? 70;
      setSearchPinned(window.scrollY + headerHeight + 12 >= pinPoint);
    };
    updatePinnedSearch();
  ...
  return (
    <div className="pcx-home">
          ...
          <div className="pcx-shell pcx-hero__grid">
            <div className="pcx-hero__copy">
              <h1 id="pcx-title">'''Do not make any visual modifications. The phrases I write are commands to understand what I want, not to be written down. Understand their content well, then execute what is required.'''
                                            ...
                                            ANALISE O CONTRATES DAS IMAGENS NO MODO CLARO NA HOMEPAGE</h1>
        ...
        <section className="pcx-market" aria-labelledby="oportunidades-title"><div className="pcx-shell">
          ...
          <div className="pcx-products" aria-busy={catalogLoading}>{catalogLoading && <span className="pcx-sr-only" role="status" aria-live="polite">Carregando oportunidades…</span>}{catalogLoading ? Array.from({ length: 5 }, (_, index) => <div className="pcx-product pcx-product--skeleton" key={index} aria-hidden="true"><span /><i /><i /></div>) : opportunities.map((product, index) => {
            ...
            return <button key={`${product.id}-${index}`} className="pcx-product" type="button" onClick={() => openProduct(product)} aria-label={`Comparar preços de ${product.name}`} title={`Comparar ${product.name}`}>
              ...
              <span className="pcx-product__body"><small>{cleanMeta(product.brand, product.size) || "Produto local"}</small><strong title={product.name}>{product.name}</strong><span className="pcx-product__store"><Store aria-hidden="true" /><b>{offer.establishment || "Loja local"}</b></span><span className="pcx-product__price"><span><small>a partir de</small><b>{money(product.minPrice)}</b></span><span className="pcx-product__action" aria-hidden="true"><ArrowRight /></span></span></span>
            </button>;
          })}</div>
        </div></section>
      ...
      <footer className="pcx-footer">
        <div className="pcx-shell pcx-footer__inner">
          <Link className="pcx-brand" to="/" aria-label="PreçoCerto — página inicial">
          ...
          <nav aria-label="Links do rodapé">
            ...
            <Link to="/lojista">Lojistas</Link>
          </nav>
          <div className="pcx-footer__meta">
            <small>© {new Date().getFullYear()} PreçoCerto</small>
            <button className="pcx-footer__credit pc-dev-credit" type="button" aria-haspopup="dialog" aria-label="Sobre o desenvolvedor Franc D'nis" onClick={() => window.dispatchEvent(new Event("precocerto:developer-about"))}>
              dev&gt;<strong>Franc D&apos;nis</strong>
            </button>
          </div>
        </div>
      </footer>
