...
export async function saveAdminEstablishment(input:{id?:string|null;name:string;neighborhood?:string;kind?:string;slug?:string;shortDescription?:string;logoUrl?:string;isVerified?:boolean}){
  if(!supabase) return {data:null,error:'Supabase indisponível'};
  const {data,error}=await supabase.rpc('admin_save_establishment',{_id:input.id||null,_name:input.name,_neighborhood:input.neighborhood||null,_kind:input.kind||null,_slug:input.slug||null,_short_description:input.shortDescription||null,_logo_url:input.logoUrl||null,_is_verified:Boolean(input.isVerified)});
  return {data:data as string|null,error:error?.message??null};
}
