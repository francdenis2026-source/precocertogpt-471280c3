// Papéis e permissões do PreçoCerto.
// REGRA DE SEGURANÇA: a autorização real é verificada no backend (RLS + função
// SECURITY DEFINER `has_role`). Nada em localStorage concede acesso administrativo.

import { supabase } from "./supabase";
export { supabase };

export type AppRole =
  | "super_admin"
  | "admin"
  | "moderator"
  | "merchant_owner"
  | "merchant_staff"
  | "consumer";

export const roleLabels: Record<AppRole, string> = {
  super_admin: "Superadministrador",
  admin: "Administrador",
  moderator: "Moderador",
  merchant_owner: "Responsável por estabelecimento",
  merchant_staff: "Equipe do estabelecimento",
  consumer: "Consumidor",
};

export const adminRoles: AppRole[] = ["super_admin", "admin", "moderator"];

export type SessionProfile = {
  userId: string;
  email: string | null;
  name: string;
  roles: AppRole[];
  isAdmin: boolean;
};

function announceAuthChange() {
  if (typeof window !== "undefined") window.dispatchEvent(new Event("pc:auth-changed"));
}

/** Lê a sessão atual, o perfil e os papéis no banco. */
export async function loadSessionProfile(): Promise<SessionProfile | null> {
  if (!supabase) return null;

  const { data: sessionData } = await supabase.auth.getSession();
  const user = sessionData.session?.user;
  if (!user) return null;

  let roles: AppRole[] = [];
  const { data: roleRows, error: rolesError } = await supabase
    .from("user_roles")
    .select("role")
    .eq("user_id", user.id);
  if (!rolesError && roleRows) roles = roleRows.map(row => row.role as AppRole);

  let profileName = "";
  const { data: profileRows } = await supabase
    .from("profiles")
    .select("full_name")
    .eq("user_id", user.id)
    .limit(1);
  if (profileRows?.[0]?.full_name) profileName = String(profileRows[0].full_name).trim();

  const metadataName = typeof user.user_metadata?.name === "string" ? user.user_metadata.name.trim() : "";
  const name = profileName || metadataName || user.email?.split("@")[0] || "Consumidor";

  return {
    userId: user.id,
    email: user.email ?? null,
    name,
    roles: roles.length ? roles : ["consumer"],
    isAdmin: roles.some(role => adminRoles.includes(role)),
  };
}

/** Verificação autoritativa no backend: `public.has_role(auth.uid(), role)`. */
export async function hasRole(role: AppRole): Promise<boolean> {
  if (!supabase) return false;
  const { data: sessionData } = await supabase.auth.getSession();
  const userId = sessionData.session?.user?.id;
  if (!userId) return false;

  const { data, error } = await supabase.rpc("has_role", { _user_id: userId, _role: role });
  if (error) return false;
  return data === true;
}

/**
 * Login do consumidor. O próprio Supabase Auth é a fonte autoritativa para saber
 * se a conta existe: só há sessão quando e-mail e senha pertencem a um cadastro válido.
 */
export async function signIn(email: string, password: string) {
  if (!supabase) return { error: "Autenticação indisponível: banco não configurado." };
  const normalizedEmail = email.trim().toLocaleLowerCase("pt-BR");
  const { data, error } = await supabase.auth.signInWithPassword({ email: normalizedEmail, password });

  if (error || !data.user || !data.session) {
    const text = (error?.message || "").toLocaleLowerCase("pt-BR");
    if (text.includes("invalid login") || text.includes("invalid credentials")) {
      return { error: "Cadastro não encontrado ou senha incorreta. Confira os dados ou crie sua conta." };
    }
    if (text.includes("email not confirmed")) {
      return { error: "Seu cadastro existe, mas o e-mail ainda precisa ser confirmado." };
    }
    return { error: error?.message || "Não foi possível validar este cadastro." };
  }

  announceAuthChange();
  return { error: null };
}

export async function signInMerchantWithCpf(cpf: string, pin: string) {
  if (!supabase) return { error: "Autenticação indisponível: banco não configurado." };
  try {
    const { data, error } = await supabase.functions.invoke("merchant-cpf-login", { body: { cpf, pin } });
    if (error) return { error: error.message || "Erro de conexão com o servidor de autenticação." };
    if (!data || data.error) return { error: data?.error || "CPF ou PIN incorretos." };
    if (!data.access_token || !data.refresh_token) return { error: "Resposta de autenticação inválida." };

    const { error: sessionError } = await supabase.auth.setSession({
      access_token: data.access_token,
      refresh_token: data.refresh_token,
    });
    if (!sessionError) announceAuthChange();
    return { error: sessionError?.message ?? null };
  } catch {
    return { error: "Ocorreu um erro inesperado ao tentar fazer login." };
  }
}

export async function signUp(email: string, password: string, name: string) {
  if (!supabase) return { error: "Cadastro indisponível: banco não configurado." };
  const cleanName = name.trim();
  const { data, error } = await supabase.auth.signUp({
    email: email.trim().toLocaleLowerCase("pt-BR"),
    password,
    options: { data: { name: cleanName } },
  });
  if (error) return { error: error.message };
  if (!data.user) return { error: "Não foi possível concluir o cadastro." };
  announceAuthChange();
  return { error: null };
}

export async function requestPasswordReset(email: string) {
  if (!supabase) return { error: "Recuperação indisponível: banco não configurado." };
  const { error } = await supabase.auth.resetPasswordForEmail(email, {
    redirectTo: `${window.location.origin}/redefinir-senha`,
  });
  return { error: error?.message ?? null };
}

export async function signOut() {
  if (supabase) await supabase.auth.signOut();
  announceAuthChange();
}

export async function syncFavorites(productIds: string[]): Promise<void> {
  if (!supabase) return;
  const { data: { session } } = await supabase.auth.getSession();
  if (!session?.user) return;

  const userId = session.user.id;
  const { data: remoteFavs } = await supabase
    .from("user_favorites")
    .select("product_id")
    .eq("user_id", userId);

  const remoteIds = new Set(remoteFavs?.map(f => f.product_id) || []);
  const localIds = new Set(productIds);
  const toInsert = productIds.filter(id => !remoteIds.has(id)).map(id => ({ user_id: userId, product_id: id }));
  const toRemove = Array.from(remoteIds).filter(id => !localIds.has(id));

  if (toInsert.length > 0) await supabase.from("user_favorites").insert(toInsert);
  if (toRemove.length > 0) await supabase.from("user_favorites").delete().in("product_id", toRemove).eq("user_id", userId);
}

export async function loadRemoteFavorites(): Promise<string[]> {
  if (!supabase) return [];
  const { data: { session } } = await supabase.auth.getSession();
  if (!session?.user) return [];

  const { data, error } = await supabase
    .from("user_favorites")
    .select("product_id")
    .eq("user_id", session.user.id);

  if (error) return [];
  return data.map(f => f.product_id);
}
