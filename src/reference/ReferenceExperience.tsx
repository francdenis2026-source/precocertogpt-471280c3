...
export function ReferenceHome() {
  ...
  return <div className="ref-page ref-home">
    ...
    <main id="conteudo-principal">
      ...
      <section className="ref-economy"><div className="ref-shell ref-economy__grid"><div><span className="ref-kicker"><TrendingDown /> SUA ECONOMIA</span><h2>Um recibo claro.<br />Uma decisão melhor.</h2><p>Compare a mesma lista em diferentes lojas e veja, em valores reais, quanto pode permanecer no seu bolso.</p><Link to="/cesta-basica">Montar lista de compras <ArrowRight /></Link></div><aside className="ref-receipt" aria-label="Simulação de economia da lista"><header className="ref-receipt__top"><div><strong>PREÇO<span>CERTO</span></strong><small>Comparação local de preços</small></div><span>SIMULAÇÃO</span></header><div className="ref-receipt__meta"><span>Feijó · Acre</span><span>Lista comparada hoje</span></div><div className="ref-receipt__columns"><span>Produto e menor preço</span><span>Valor</span></div>{receipt.map(product => <div className="ref-receipt__item" key={product.id}><div><span>{product.name}</span><small>{product.establishment}</small></div><strong>{brl.format(product.minPrice)}</strong><em>Você evita {brl.format(Math.max(0, product.maxPrice - product.minPrice))}</em></div>)}<div className="ref-receipt__summary"><div><span>Menor total encontrado</span><strong>{brl.format(receipt.reduce((sum, item) => sum + item.minPrice, 0))}</strong></div><div><span>Maior total comparado</span><strong>{brl.format(receipt.reduce((sum, item) => sum + item.maxPrice, 0))}</strong></div></div><div className="ref-receipt__total"><div><span>Economia possível</span><small>escolhendo os menores preços</small></div><strong>{brl.format(receipt.reduce((sum, item) => sum + Math.max(0, item.maxPrice - item.minPrice), 0))}</strong></div><footer className="ref-receipt__note"><BadgeCheck /><span>Preços locais verificados.<small>Consulte a atualização antes da compra.</small></span></footer><div className="ref-receipt__barcode" aria-hidden="true" /></aside></div></section>
