      ...
      {mode === "new" ? <form className="store-product-form" onSubmit={saveNewProduct}>
        ...
        <div className="store-product-table">
          ...
          {visibleRows.map(({ product, price, storeCount, minPrice }) => <div className="store-product-row" key={product.id}>
            ...
            <span>{mode === "missing" ? <><b>{storeCount} {storeCount === 1 ? "estabelecimento" : "estabelecimentos"}</b><small>A partir de {money(minPrice)}</small></> : <><b>{money(Number(price?.value ?? 0))}</b><small>{price?.captured_at ? new Date(price.captured_at).toLocaleDateString("pt-BR") : "Sem data"}</small></>}</span>
