...
import { ArrowRight, BadgeCheck, BrainCircuit, CheckCircle2, ChevronRight, ExternalLink, Heart, Moon, PackageSearch, Scale, Search, Sparkles, Store, Sun, TrendingDown, X } from "lucide-react";
...
export function SmartCompareSearchProMax(){
 ...
 useEffect(()=>{if(!selectedProduct)return;const previous=document.body.style.overflow;document.body.style.overflow="hidden";const close=(event:KeyboardEvent)=>{if(event.key==="Escape")setSelectedProduct(null)};window.addEventListener("keydown",close);return()=>{document.body.style.overflow=previous;window.removeEventListener("keydown",close)}},[selectedProduct]);
 ...
 return <main className="scpm-page">
  ...
  {selectedProduct&&createPortal(<div className="scpm-modal" role="dialog" aria-modal="true" aria-labelledby="scpm-product-title"><button className="scpm-modal__backdrop" type="button" aria-label="Fechar detalhes do produto" onClick={()=>setSelectedProduct(null)}/><article className="scpm-modal__card"><button className="scpm-modal__close" type="button" aria-label="Fechar" onClick={()=>setSelectedProduct(null)}><X aria-hidden="true"/></button><div className="scpm-modal__media"><ProductThumb product={selectedProduct}/></div><div className="scpm-modal__body"><small>{selectedProduct.category||"Produto"}</small><h2 id="scpm-product-title">{selectedProduct.name}</h2><p>{selectedProduct.brand}{selectedProduct.size?` · ${selectedProduct.size}`:""}</p><div className="scpm-modal__prices"><span><small>Menor preço</small><strong>{money(selectedProduct.minPrice)}</strong></span><span><small>Média encontrada</small><strong>{money(selectedProduct.avgPrice)}</strong></span></div><div className="scpm-modal__offers" aria-label="Preços por estabelecimento">{(selectedProduct.offers?.length?selectedProduct.offers:[{establishment:selectedProduct.establishment,neighborhood:selectedProduct.neighborhood,value:selectedProduct.minPrice}]).slice().sort((a,b)=>a.value-b.value).slice(0,5).map((offer,index)=><div className="scpm-modal__offer" key={`${offer.establishment}-${offer.value}-${index}`}><span><b>{offer.establishment}</b><small>{offer.neighborhood||"Feijó-AC"}</small></span><strong>{money(offer.value)}</strong></div>)}</div><div className="scpm-modal__actions"><a className="scpm-modal__primary" href={`/produto/${selectedProduct.slug||selectedProduct.id}`}>Ver página completa <ExternalLink aria-hidden="true"/></a><a className="scpm-modal__secondary" href={`/cesta-basica?produto=${encodeURIComponent(String(selectedProduct.id))}`}>Adicionar à cesta <ArrowRight aria-hidden="true"/></a></div></div></article></div>,document.body)}
 </main>;
}
