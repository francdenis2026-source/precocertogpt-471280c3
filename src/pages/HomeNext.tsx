  ...
  return <div className="pcn-home pc-next" ref={pageRef}>
    ...
    <main>
      <section className="pcn-hero"><div className="pcn-hero__photo" aria-hidden="true" /><div className="pcn-hero__veil" aria-hidden="true" /><div className="pcn-shell pcn-hero__content"><div className="pcn-hero__copy"><span className="pcn-eyebrow"><CheckCircle2 aria-hidden="true" /> Ao vivo em Feijó · preços verificados</span><h1>Compare antes<br /><span>de comprar.</span></h1><p>Preços locais organizados para você decidir melhor — antes de sair de casa.</p>
        <div className="pcn-search-shell"><form className="pcn-search" onSubmit={submitSearch} role="search"><Search aria-hidden="true" /><input id="pcn-home-search" value={query} onFocus={() => hasSearchQuery && setSearchDialogOpen(true)} onChange={(event) => updateSearchQuery(event.target.value)} placeholder="Busque produto, marca ou estabelecimento" aria-label="Buscar produto, marca ou estabelecimento" autoComplete="off" />{hasSearchQuery && <button className="pcn-search__clear" type="button" onClick={clearSearch} aria-label="Limpar pesquisa"><X aria-hidden="true" /></button>}<button className="pcn-search__submit" type="submit">Buscar <ArrowRight aria-hidden="true" /></button></form>
        ...
        {heroProduct && <aside className="pcn-receipt" aria-label="Resumo de economia do produto em destaque"><div className="pcn-receipt__top"><span>RECIBO DE ECONOMIA · AGORA</span><BadgePercent aria-hidden="true" /></div><small>Escolhendo o menor preço, você economiza</small><strong className="pcn-receipt__saving">{money(heroSaving)}</strong><p>{heroProduct.name}</p><div className="pcn-receipt__rows"><span>Menor preço <b>{money(heroProduct.minPrice)}</b></span><span>Maior preço <b>{money(heroProduct.maxPrice)}</b></span>{heroProduct.offers?.length ? <span>Comparado em <b>{heroProduct.offers.length} estabelecimentos</b></span> : <span>Oferta em <b>{heroProduct.establishment || "comércio local"}</b></span>}</div><button type="button" onClick={() => navigate(`/buscar?q=${encodeURIComponent(heroProduct.name)}`)}>Abrir comparação <ArrowRight aria-hidden="true" /></button><em>Coleta local · verifique antes de sair</em></aside>}
      </div></section>
      <section className="pcn-metrics-wrap" aria-label="Resumo do catálogo"><div className="pcn-shell pcn-metrics"><div><span className="pcn-metric-icon is-blue"><Tag /></span><strong>{catalog.metrics.prices.toLocaleString("pt-BR")}</strong><small>Preços verificados</small></div><div><span className="pcn-metric-icon is-amber"><ShoppingBasket /></span><strong>{catalog.metrics.products.toLocaleString("pt-BR")}</strong><small>Produtos cadastrados</small></div><div><span className="pcn-metric-icon is-green"><BadgePercent /></span><strong>{heroSaving > 0 ? money(heroSaving) : "—"}</strong><small>Diferença encontrada agora</small></div><div><span className="pcn-metric-icon is-violet"><Store /></span><strong>{catalog.metrics.stores}</strong><small>Estabelecimentos parceiros</small></div><div><span className="pcn-metric-icon is-amber"><CheckCircle2 /></span><strong>Feijó-AC</strong><small>Nossa cidade</small></div></div></section>
      <section className="pcn-section pcn-shell"><div className="pcn-section__head"><div><h2>O comércio de Feijó, lado a lado.</h2><p>Abra um estabelecimento para ver catálogo, atualização e preços disponíveis.</p></div><Link to="/estabelecimentos">Ver todos os estabelecimentos <ArrowRight /></Link></div><div className="pcn-stores">{featuredStores.map((store) => <Link className="pcn-store" key={store.id} to={`/estabelecimento/${store.slug}`}><span className="pcn-store__mark" style={{ background: store.color }}>{store.name.slice(0, 1)}</span><strong>{store.name}</strong><small>{store.neighborhood || "Feijó"}</small><span>{store.products} produtos <ArrowRight /></span></Link>)}</div></section>
