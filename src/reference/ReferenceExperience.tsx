...
function PublicHeader({ current }: { current?: "home" | "search" | "basket" | "stores" | "profile" }) {
  ...
  return <header className="ref-header"><div className="ref-shell ref-header__inner"><Brand />
    ...
    <nav className="ref-nav" aria-label="Navegação principal"><Link className={current === "home" ? "is-active" : ""} to="/">Início</Link><Link to="/explorar">Setores</Link><Link className={current === "search" ? "is-active" : ""} to="/buscar">Buscar</Link><Link className={current === "stores" ? "is-active" : ""} to="/estabelecimentos">Lojas</Link><Link className={current === "basket" ? "is-active" : ""} to="/cesta-basica">Lista {count > 0 && <b>{count}</b>}</Link></nav>
    <div className="ref-header__actions"><ThemeButton /><Link className="ref-favorites-link" to="/favoritos" aria-label="Favoritos"><Heart />{/* O contador de favoritos poderia ser injetado aqui */}</Link><Link className="ref-signin" to="/login">Entrar</Link><button type="button" className="ref-menu" aria-label={menu ? "Fechar menu" : "Abrir menu"} aria-expanded={menu} onClick={() => setMenu(value => !value)}>{menu ? <X /> : <Menu />}</button></div>
  </div>{menu && <nav className="ref-mobile-menu"><Link to="/explorar" onClick={() => setMenu(false)}>Explorar setores</Link><Link to="/buscar" onClick={() => setMenu(false)}>Buscar no PreçoCerto</Link><Link to="/estabelecimentos" onClick={() => setMenu(false)}>Estabelecimentos</Link><Link to="/cesta-basica" onClick={() => setMenu(false)}>Lista de compras</Link><Link to="/favoritos" onClick={() => setMenu(false)}>Meus Favoritos</Link><Link to="/lojista" onClick={() => setMenu(false)}>Para negócios</Link><div className="ref-mobile-menu__footer"><ThemeButton /><span>Alterar tema</span></div></nav>}</header>;
}

function PublicFooter() {
  return <footer className="ref-footer"><div className="ref-shell ref-footer__inner"><div className="ref-footer__identity"><Brand inverse /><p>O preço certo perto de você.</p></div><nav aria-label="Navegação do rodapé"><div><strong>Plataforma</strong><Link to="/buscar">Comparar Preços</Link><Link to="/explorar">Setores</Link><Link to="/estabelecimentos">Lojas Locais</Link><Link to="/cesta-basica">Lista de Compras</Link></div><div><strong>Negócios</strong><Link to="/lojista">Seja um Parceiro</Link><Link to="/quero-vender">Quero Vender</Link><Link to="/painel-lojista">Painel Lojista</Link></div><div><strong>Suporte</strong><Link to="/fale-conosco">Fale Conosco</Link><Link to="/colaborar">Colaborar</Link><Link to="/meus-pedidos">Rastrear Pedido</Link></div></nav><div className="ref-footer__meta"><span><BadgeCheck /> Preços locais verificados</span><small>&copy; 2026 PreçoCerto</small></div></div></footer>;
}
...
export function ReferenceHome() {
  ...
  return <div className="ref-page ref-home"><PublicHeader current="home" /><div className="ref-evolution-badge" role="status" aria-label="Aviso: site em construção"><span aria-hidden="true" /><strong>Site em construção</strong></div><main id="conteudo-principal">
...
export function ReferenceProductPage() {
  const { identifier = "" } = useParams(); const { catalog, loading: catalogLoading } = useCatalogState(); const { isFavorite, toggleFavorite } = useFavorites(); const basket = useBasket(); const product = useMemo(() => catalog.products.find(item => String(item.id) === identifier || item.slug === identifier), [catalog.products, identifier]);
...
export function ReferenceStorePage() {
  const { identifier = "" } = useParams(); const catalog = useCatalog(); const store = catalog.stores.find(item => String(item.id) === identifier || item.slug === identifier) || catalog.stores[0]; const products = catalog.products.filter(item => item.offers?.some(offer => String(offer.establishmentId) === String(store?.id)) || String(item.establishmentId) === String(store?.id)); if (!store) return <ReferenceNotFound />;
  return <div className="ref-page ref-directory"><PublicHeader current="stores" /><main id="conteudo-principal" className="ref-shell ref-directory__main"><Link className="ref-backline" to="/estabelecimentos"><ArrowLeft /> Todos os estabelecimentos</Link><section className="ref-store-hero"><i style={{ background: store.color }}><Store /></i><div><span>PARCEIRO VERIFICADO · {store.neighborhood}</span><h1>{store.name}</h1><p>{store.products} produtos monitorados · preços atualizados hoje</p></div><BadgeCheck /></section><section className="ref-results"><header><div><span>CATÁLOGO LOCAL</span><h2>Preços deste estabelecimento</h2></div><small>Compare antes de sair</small></header><div className="ref-product-grid">{(products.length ? products : catalog.products.slice(0, 6)).map(product => <Link key={product.id} to={`/produto/${product.slug}`}><div><ProductVisual product={product} /></div><small>{product.category}</small><strong>{product.name}</strong><span>{product.size}</span><footer><em>a partir de</em><b>{brl.format(product.minPrice)}</b></footer></Link>)}</div></section></main><PublicFooter /><AppDock current="stores" /></div>;
}

export function ReferenceBasketPage() {
  const catalog = useCatalog(); const basket = useBasket(); const rows = basket.items.map(entry => ({ entry, product: catalog.products.find(item => String(item.id) === entry.productId) })).filter(row => row.product); const total = rows.reduce((sum, row) => sum + (row.product?.minPrice || 0) * row.entry.quantity, 0); const highest = rows.reduce((sum, row) => sum + (row.product?.maxPrice || 0) * row.entry.quantity, 0); const savings = Math.max(0, highest - total);
  return <div className="ref-page ref-directory ref-basket-page"><PublicHeader current="basket" /><main id="conteudo-principal" className="ref-shell ref-directory__main"><section className="ref-basket-hero"><div><span>SUA LISTA INTELIGENTE</span><h1>Planeje melhor.<br />Compre com clareza.</h1><p>Monte sua cesta, compare os valores locais e descubra quanto pode economizar antes de sair de casa.</p><Link to="/buscar">Adicionar produtos <ArrowRight /></Link></div><aside><ShoppingBasket /><span><small>{basket.count ? "ITENS NA CESTA" : "COMECE SUA LISTA"}</small><strong>{basket.count || "—"}</strong><em>{basket.count ? `Economia estimada: ${brl.format(savings)}` : "Compare produtos de Feijó"}</em></span></aside></section>{!rows.length ? <section className="ref-basket-empty"><div><span><ShoppingBasket /></span><small>LISTA PRONTA PARA COMEÇAR</small><h2>Sua próxima economia começa aqui.</h2><p>Pesquise produtos, adicione à cesta e veja automaticamente a melhor combinação de preços.</p><Link to="/buscar">Explorar produtos <ArrowRight /></Link></div><ol><li><i>1</i><span><strong>Encontre</strong><small>Busque o produto desejado</small></span></li><li><i>2</i><span><strong>Adicione</strong><small>Monte sua lista de compras</small></span></li><li><i>3</i><span><strong>Compare</strong><small>Veja onde economizar</small></span></li></ol></section> : <div className="ref-basket-layout"><section className="ref-basket-list"><header><div><span>PRODUTOS SELECIONADOS</span><h2>Sua lista de compras</h2></div><strong>{rows.length} {rows.length === 1 ? "produto" : "produtos"}</strong></header>{rows.map(({ entry, product }) => product && <article key={product.id}><div className="ref-basket-product"><i><ProductVisual product={product} /></i><span><small>{product.category}</small><strong>{product.name}</strong><em>{product.size}</em></span></div><div className="ref-quantity"><button type="button" aria-label={`Diminuir ${product.name}`} onClick={() => basket.update(product.id, -1)}>{entry.quantity === 1 ? <Trash2 /> : <Minus />}</button><strong>{entry.quantity}</strong><button type="button" aria-label={`Aumentar ${product.name}`} onClick={() => basket.update(product.id, 1)}><Plus /></button></div><div className="ref-basket-price"><small>melhor preço</small><strong>{brl.format(product.minPrice * entry.quantity)}</strong><em>{product.establishment}</em></div></article>)}</section><aside className="ref-basket-summary"><span>RESUMO DA COMPARAÇÃO</span><h2>Sua economia</h2><div><span>Melhor combinação</span><strong>{brl.format(total)}</strong></div><div><span>Compra sem comparar</span><strong>{brl.format(highest)}</strong></div><footer><span>Economia possível</span><strong>{brl.format(savings)}</strong><small><BadgeCheck /> preços verificados hoje</small></footer><Link to="/estabelecimentos">Ver onde comprar <ArrowRight /></Link></aside></div>}</main><PublicFooter /><AppDock current="basket" /></div>;
}
...
export function ReferenceAdminDashboard() {
  const [authorized, setAuthorized] = useState<boolean | null>(null); const [summary, setSummary] = useState({ gmvToday: 0, platformRevenueToday: 0, subscriptionRevenueMonth: 0, commissionRevenueToday: 0, activeMerchants: 0, ordersToday: 0, cancelledToday: 0, averageTicket: 0 }); useEffect(() => { void (async () => { const profile = await loadSessionProfile(); setAuthorized(Boolean(profile?.isAdmin)); if (profile?.isAdmin) setSummary(await loadPlatformSummary()); })(); }, []); if (authorized === null) return <main className="ref-admin-state"><ShieldCheck /><h1>Verificando acesso…</h1></main>; if (!authorized) return <main className="ref-admin-state"><ShieldCheck /><h1>Acesso administrativo restrito</h1><p>Entre com uma conta administrativa autorizada.</p><Link to="/login?redirect=/admin/plataforma">Entrar com segurança</Link></main>; const cards = [["Economia verificada", brl.format(summary.gmvToday), TrendingDown], ["Comerciantes ativos", String(summary.activeMerchants), Store], ["Pedidos hoje", String(summary.ordersToday), ShoppingBasket], ["Ticket médio", brl.format(summary.averageTicket), CircleDollarSign]] as const;
